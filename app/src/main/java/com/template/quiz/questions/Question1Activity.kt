...
class Question1Activity : AppCompatActivity() {
    ...
    private fun setOnClicks() {
        answ1.setOnClickListener {
            if (1 == QuestionManager.getAnswer(1))
        ...
        answ2.setOnClickListener {
            if (2 == QuestionManager.getAnswer(1))
                actionsIfAnswerIsCorrent()
            else
                actionsIfAnswerIsWrong(it)
        }
        ...
        answ3.setOnClickListener {
            if (3 == QuestionManager.getAnswer(1))
                actionsIfAnswerIsCorrent()
            else
                actionsIfAnswerIsWrong(it)
        }
        ...
        answ4.setOnClickListener {
            if (4 == QuestionManager.getAnswer(1))
                actionsIfAnswerIsCorrent()
            else
                actionsIfAnswerIsWrong(it)
    ...
    private fun showAnswDialog() {
        val answer = "The Olympic Flame as a symbol of the modern Olympic movement was introduced by the architect Jan Wils who designed the stadium for the 1928 Summer Olympics in Amsterdam."
